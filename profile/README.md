...
|개표방법|1. 투표방 생성 시 투표종료 시간을 투표스케줄러 서버로 전송. 2. 투표종료 서버는 투표 종료시간을 데이터베이스에 저장한다. 3. 투표스케줄러 서버는 주기적으로(10분) 데이터베이스에 저장된 투표종료시간을 점검. 4. 투표 종료시간이 넘은 투표방은 메세지큐를 통해 개표서버로 전달. 5. 개표서버는 블록체인에 있는 해당 투표방 정보를 가져와 개표 후 웹서버로 전달한다.
